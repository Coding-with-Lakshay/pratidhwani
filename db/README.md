...
```
   ...
   |- /pb_data  -> GCS Fuse mount of  gs://dmjone-pratidhwani-db
...
```bash
# from the repo root
PROJECT_ID=dmjone bash db/deploy.sh
```
...
```bash
gcloud secrets versions access latest --secret=pratidhwani-pb-admin --project=dmjone
```
...
```bash
gcloud run services proxy pratidhwani-db --region=asia-south1 --project=dmjone --port=8787
```
...
printf '%s' "$NEW_PW" | gcloud secrets versions add pratidhwani-pb-admin \
  --data-file=- --project=dmjone
gcloud run services update pratidhwani-db \
  --region=asia-south1 --project=dmjone \
  --set-secrets=PB_ADMIN_PASSWORD=pratidhwani-pb-admin:latest
```
...
```bash
gcloud run services logs read pratidhwani-db --region=asia-south1 --project=dmjone --limit=50
```
